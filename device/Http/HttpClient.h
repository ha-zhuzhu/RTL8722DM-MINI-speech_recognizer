...
public:
  ...
  */
  ...
  int mypost(const char *aServerName,
             const char *aURLPath,
             SdFatFile file,
             const char *aUserAgent = NULL);
...
@return 0 if successful, else error
*/
  int mystartRequest(const char *aServerName,
                     uint16_t aServerPort,
                     const char *aURLPath,
                     const char *aHttpMethod,
                     const char *aUserAgent);
  ...
  /** Send the first part of the request and the initial headers.
    ...
    @return 0 if successful, else error
  */
  int mysendInitialHeaders(const char *aServerName,
                           IPAddress aServerIP,
                           uint16_t aPort,
                           const char *aURLPath,
                           const char *aHttpMethod,
  ...
  int mysetPostData(char *post_start, char *post_end, uint8_t *post_content, int post_start_len,
                    int post_end_len, int post_content_len, char *content_type);

  int mysetPostContent(uint8_t *post_content_d, int post_content_len_d, int post_len_d);
...
protected:
  ...
  */
  ...
  uint8_t post_start_len;
  uint8_t post_end_len;
